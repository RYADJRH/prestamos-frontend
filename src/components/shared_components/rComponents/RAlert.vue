...
<template>
  <div class="z-[75] inset-0 fixed" v-if="dialogStore.open">
    <div class="flex items-center justify-center min-h-screen px-4 py-2">
      <div
        class="bg-white border shadow-xl md:w-1/3 w-full p-4 rounded-md flex items-center flex-col gap-y-4 dark:bg-gray-700 dark:text-white">
        <component :is="dialogStore.icon" class="h-24 w-24" :class="{
        ...
        <p class="text-md text-gray-600 min-h-[5rem] text-center dark:text-white">
          {{ dialogStore.optionsDefault.description }}
        </p>
        <div>
          <r-btn @click="dialogStore.cancel" class="mr-3" variant="danger" v-if="dialogStore.optionsDefault.confirm">
            Cancelar
          </r-btn>

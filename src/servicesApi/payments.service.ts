...
const apiPaymentsBorrowerPersonalLoan = async (
...
const apiReportePaymentsBeneficiaryPersonalLoan = async (
  beneficiary: number,
  date: string,
  status: string
) => {
...
const apiAdjustPaymentGroup = async (paymentId: number, amount_payment: number) => {
  return await axios.patch(`${PREFIX_API}payments/group/adjust/${paymentId}`, { amount_payment });
};

const apiAdjustPayment = async (paymentId: number, amount_payment: number) => {
  return await axios.patch(`${PREFIX_API}payments/personal-loans/adjust/${paymentId}`, { amount_payment });
};

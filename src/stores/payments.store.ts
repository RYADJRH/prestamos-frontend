...
const usePaymentStore = defineStore('payments', {
  ...
  getters: {
    ...
    getFullAdjusment(state) {
      return state.fullAdjusment;
    },
  },
  actions: {
    ...
    async getApiPaymentsBorrower(slug_group: string, slug_borrower: string, page: number) {
      return await apiPaymentsBorrower(slug_group, slug_borrower, page)
        .then((response) => {
          const paymentsResponse = response.data.payments;
          const totalOwe = response.data.total;
          const fullAdjusment = response.data.full_adjusment;
          ...
          this.payments.data = paymentsResponse.data;
          this.total_owe = totalOwe;
          this.fullAdjusment = fullAdjusment;
    ...
    async reportePaymentsBeneficiaryPersonalLoan(
      beneficiary: number,
      date: string,
      status: { type: string; typeName: string; selected: boolean }[]
    ) {
    ...
    async reportePaymentsBydateGroup(
      slug_group: string,
      date: string,
      status: { type: string; typeName: string; selected: boolean }[]
    ) {
    ...
    async adjustPayment(paymentId: number, amount_payment: number, isGroup: boolean = false) {
      return isGroup
        ? await apiAdjustPaymentGroup(paymentId, amount_payment)
            .then(() => {
        ...
        : await apiAdjustPayment(paymentId, amount_payment)
            .then(() => {
              return Promise.resolve(true);
            })
            .catch((err) => {

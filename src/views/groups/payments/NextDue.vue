...
async function fnPaymentsNextDue() {
    await paymentStore.getApiPaymentsNextDue(slug.value, 1, '')
        .catch(() => { });
}
        ...
        <div class="flex justify-between">
            <div class="flex items-center gap-2">
                <a class="text-gray-600 hover:text-red-800 cursor-pointer  dark:text-white dark:hover:text-gray-300"
                    @click="router.push(`/grupo/${slug}`)">
                    <ArrowSmallLeftIcon class="w-8 h-8"></ArrowSmallLeftIcon>
                </a>
                <h1 class="text-2xl text-gray-600 dark:text-white">Pagos siguientes - {{ group.name }}</h1>
            </div>

        </div>
        <info-payments class="mt-4"></info-payments>
        <view-payments class="mt-8" type="next-due-group"></view-payments>
    </div>
</template>